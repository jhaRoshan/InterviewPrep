...
public class MaxHeap<K extends Comparable<K>>  implements Heap<K>{
	...
	private void heapifyUp(int i) {
		...
		if(isValidIndex(parentIndex) && compareValue(parentIndex,i) < 1) {
			swap(i , parentIndex);
	...
	private void heapifyDown(int i) {
		...
		int highestChildValueIndex = getHighestChildValueIndex(leftChildIndex , rightChildIndex);
		
		if(highestChildValueIndex != -1 && heap.get(highestChildValueIndex).compareTo(heap.get(i)) > 0) {
			swap(i, highestChildValueIndex);
			heapifyDown(highestChildValueIndex);
		}
	}
	

	private int getHighestChildValueIndex(int leftChildIndex, int rightChildIndex) {
